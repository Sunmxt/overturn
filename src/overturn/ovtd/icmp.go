...
func NewOVTPacket(payload_size uint) OVTPacket {
    pack := make(OVTPacket, payload_size + OVT_HEADER_SIZE)
    copy(pack[0:4], OVT_MAGIC)
    return pack
}

func OVTPacketUnpack(buf byte[], size_limit uint32) *OVTPacket, error {
    if len(buf) < 4 {
        return false, nil, fmt.Errorf(PKG_TOO_SHORT)
    }
    if buf[0:4] != OVT_MAGIC {
        return false, nil, fmt.Errorf(PKG_INVALID)
    }
    if len(buf) < 8 {
        return true, nil, fmt.Errorf(PKG_TOO_SHORT)
    }
    enc_len := binary.BigEndian.Uint32(buf[4:8])
    if enc_len != len(buf) {
        return true, nil, fmt.Errorf("Packet size unmatched. (buffed: %v, encoded: %v)", len(buf), enc_len)
    }
...
func (pack OVTPacket) Pack() []byte {
    binary.BigEndian.PutUint32(pack[4:8], len(pack))
    return []byte(OVTPacket)
}

func (pack OVTPacket) PayloadRef() []byte {
    return pack[4:len(OVTPacket)]
}
